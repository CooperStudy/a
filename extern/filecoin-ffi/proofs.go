...
func GeneratePieceCIDFromFile(proofType abi.RegisteredSealProof, pieceFile *os.File, pieceSize abi.UnpaddedPieceSize) (cid.Cid, error) {
	...
	//CommP [50 230 186 185 32 165 136 103 13 80 191 116 179 206 75 99 225 39 105 237 231 120 159 20 22 10 175 95 120 244 66 50]
	fmt.Println("CommP",hex.EncodeToString(resp.CommP[:])) //968ea14972b6d69a6007c6e02616eec352677ab5360ed6e72b1854e86975a604
	fmt.Println("CommP string",string(resp.CommP[:]),len(resp.CommP))//32
...
func GenerateWinningPoStSectorChallenge(proofType abi.RegisteredPoStProof, minerID abi.ActorID, randomness abi.PoStRandomness, eligibleSectorsLen uint64) ([]uint64, error) {
	//GenerateWinningPoStSectorChallenge(abi.RegisteredPoStProof_StackedDrgWinning2KiBV1, minerID, randomnessFr32[:], eligibleSectorsLen=1)
	proverID, err := toProverID(minerID)
	if err != nil {
		return nil, err
	}
...
// detected GPU device.
/**
// FilGpuDeviceResponse as declared in filecoin-ffi/filcrypto.h:159
...
func GetGPUDevices() ([]string, error) {
	...
	fmt.Println("====resp",resp) //====resp &{0  0 [] 0x396efe0 <nil>}
	fmt.Println("StatusCode",resp.StatusCode) //0
	fmt.Println("===devicesLen",resp.ErrorMsg)//
	resp.Deref()
...
func GetSealVersion(proofType abi.RegisteredSealProof) (string, error) {
	...
	fmt.Println("====resp",resp) //resp &{0   0x4542750 <nil>}
	fmt.Printf("resp.statusCode:%v\n",resp.StatusCode)
	fmt.Printf("ErrorMsg:%v\n",resp.StringVal) //""
	fmt.Printf("StringVal:%v\n",resp.StringVal)//""
	fmt.Printf("ref4f413043:%v\n",resp.Ref()) //&{0 <nil> 0x3c7b750}
	...
	fmt.Println("ge",generated.FCPResponseStatusFCPCallerError)//2
	if resp.StatusCode != generated.FCPResponseStatusFCPNoError {
		return "", errors.New(generated.RawString(resp.ErrorMsg).Copy())
	}

	fmt.Println("rawString",".",generated.RawString(resp.StringVal).Copy()) //StackedDrg64GiBV1_1
...
func toProverID(minerID abi.ActorID) (generated.Fil32ByteArray, error) {
	...
	fmt.Println("maddr",maddr) //t042 //t01
	if err != nil {
